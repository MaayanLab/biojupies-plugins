...
def run(enrichr_results, signature_label):
	...
	for geneset in ['upregulated', 'downregulated']:
		...
		# Append ChEA results
		enrichment_dataframe = s.get_enrichr_results(enrichr_results[geneset]['userListId'], gene_set_libraries=libraries, geneset=geneset)
...
def plot(enrichment_results, plot_counter):
	...
	# Add table
	s.results_table(enrichment_results['enrichment_dataframe'].copy(), source_label='Kinase', target_label='substrate')
	...
	s.download_button(results_txt, 'Download Results', 'kinase_enrichment_results.txt')
